    ...
    (unwind-protect
        (progn
          ...
          (unless (get-buffer buf-name)
            ...
            (while (get-process proc-name) (sleep-for 0.1)))
          (set-buffer buf-name)
          (goto-char (point-min))
          (if (looking-at (concat "No Python documentation found for '" func "'"))
              (error (buffer-substring-no-properties (match-beginning 0) (match-end 0)))
            (while (re-search-forward (concat "\nProcess " proc-name " finished\n") (point-max) t)
              (replace-match ""))
            (goto-char (point-max))
            (delete-blank-lines))
          ;; TODO: make help buffer unmodified and read-only
