...
(defun my-comint-preoutput-filter-function (STRING)
  "Perform various operations as part of `comint-preoutput-filter-functions'."
  ;; Determine, whether or not point is NOT on the last window line, but on the last buffer line.
  (let
      (
       ;; cur-point-pos
       ;; last-window-line-beginning
       ;; last-window-line-end
       ;; last-buffer-line-beginning
       ;; last-buffer-line-end
       ;; point-on-last-window-line
       ;; point-on-last-buffer-line
       screen-lines-count
       ;; whole-window-occupied
       )
    ;; (setq cur-point-pos (point))
    ;; Count screen lines in selected window.
    (setq screen-lines-count (count-screen-lines (window-start) (window-end)))
    ;; Determine whether "screen-lines-count" is less than the height of the selected window in terms of screen lines (i.e., whether the whole window height is occupied by text).
    ;; (setq whole-window-occupied (< screen-lines-count (- (window-height (selected-window) 'floor) 1)))  ;; We subtract 1 from the value returned by "window-height", since that function also considers the partial last line.
    ;; ;; Record the start and end of the last window line.
    ;; (save-excursion
    ;;   (move-to-window-line -1)
    ;;   (move-end-of-line nil)  ;; We first move to the end of the line since "move-to-window-line" apparently leaves point at the very beginning of the line, disregarding even the shell prompt string.
    ;;   (setq last-window-line-end (point))
    ;;   (move-beginning-of-line nil)
    ;;   (setq last-window-line-beginning (point)))
    ;; ;; Record the start and end of the last buffer line.
    ;; (save-excursion
    ;;   (end-of-buffer)
    ;;   (move-beginning-of-line nil)
    ;;   (setq last-buffer-line-beginning (point))
    ;;   (move-end-of-line nil)
    ;;   (setq last-buffer-line-end (point)))
    ;; ;; Store information about whether or not point is on last window line.
    ;; (setq point-on-last-window-line (and (<= last-window-line-beginning cur-point-pos) (>= last-window-line-end cur-point-pos)))
    ;; ;; Store information about whether or not point is on last buffer line.
    ;; (setq point-on-last-buffer-line (and (<= last-buffer-line-beginning cur-point-pos) (>= last-buffer-line-end cur-point-pos)))
    ;; ;; If point is NOT on the last window line, but on the last buffer line,...
    ;; (if (and (not point-on-last-window-line) point-on-last-buffer-line)
    ;;     ;; ...set variable "my-last-window-start" to the value returned by "window-start".
    ;;     (setq my-last-window-start (window-start))
    ;;   ;; Else, set variable "my-last-window-start" to nil.
    ;;   (setq my-last-window-start nil))
    ;; If "screen-lines-count" is less than the height of the selected window in terms of screen lines (i.e., if the whole window height is occupied by text),...
    (if (< screen-lines-count (- (window-height (selected-window) 'floor) 1))
        ;; ...set variable "my-last-window-start" to the value returned by "window-start".
        (setq my-last-window-start (window-start))
      ;; Else, set variable "my-last-window-start" to nil.
      (setq my-last-window-start nil))
    ;; )  TESTING
  ;; BEGIN TESTING
    (message "screen lines: %d" screen-lines-count)
    ;; (message "window height: %d" (- (window-height (selected-window) 'floor) 1))
  )
  ;; END TESTING
  ;; Return argument "STRING", since it is required by the subsequent functions listed in "comint-preoutput-filter-functions".
  STRING)

(defun my-comint-output-filter-function (STRING)
  "Perform various operations as part of `comint-output-filter-functions'."
  ;; If "my-last-window-start" is non-nil,...
  (if my-last-window-start
      ;; ...use its value in "set-window-start".
      (set-window-start (selected-window) my-last-window-start)))
